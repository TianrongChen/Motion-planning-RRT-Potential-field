...
%pause
    ...
    %hack
    temp=map_struct.map_samples{i};
    temp(temp~=0)=1;
    map_struct.map_samples{i}=temp;
    observed_map=temp;
    disp(['Map sample    ',num2str(i)] )
...
%     old_map=sign(observed_map);
    ...
    while (state.moveCount < params.max_moveCount && flags ~= 2)
        ...
        if counter_disp==1
            %first step
            result = PathRRT(params,rrts_param, state,observed_map,goal);
            display_mix;
        else
            if sum(abs(sign(observed_map)-old_map),'all')%||control_count>length(result.control)
...
%                         action=u(pp);
%                         % here target is not important
%                         [state, observed_map, flags] = motionModel(params, state, action, observed_map, observed_map, map_struct.start);
%                         if flags==2
%                                disp('Fail')
%                         end
%                         
...
%                 end
                ...
                figure(2);
                clf;
                disp("Prepare to find the way to go back")
                
                result = PathRRT(params,rrts_param, state,observed_map,map_struct.start);
                display_mix;
                control_count=1;
                old_map=sign(observed_map);
            elseif  control_count>length(result.control)
                disp("Get to the target, prepare to find the way to goal")
                control_count=1; %reset
                result = PathRRT(params,rrts_param, state,observed_map, goal);
                display_mix;
            end
        end
%       
        % ??????
        if ~isempty(result.control)  
            action=result.control(control_count);
        ...
        if (DISPLAY_ON)&&(~mod(counter_disp,10))
            display_environment;
        end
